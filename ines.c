...
void cleanup(struct iNES_HEADER *this)
{
...
int iNESLoad(const char *name, int fp, int OverwriteVidMode)
{
   ...
   // SetupCartPRGMapping(1,WRAM,8192,1); // ?
	...
	FCEU_printf(" PRG ROM:  %3d x 16KiB\n CHR ROM:  %3d x  8KiB\n ROM CRC32:  0x%08lx\n",
                (round) ? ROM_size : head.ROM_size, head.VROM_size,
				(unsigned long)iNESGameCRC32);
	...
	while(tmp->init)
	{
		if(num==tmp->number)
		{
			...
			if(!VROM_size)
			{
				...
				if((VROM = (uint8 *)FCEU_malloc(CHRRAMSize)) == NULL) return 0;
				FCEU_MemoryRand(VROM,CHRRAMSize);
